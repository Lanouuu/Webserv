...
int Request::parse_request(Client & client, Server const & server) {
    ...
    if(_methode == "POST")
    {
        ...
        if(_content_type == "application/x-www-form-urlencoded\r") // pour requete en brut
        {
            ...
            if(_url == "/srcs/cgi-bin/download.py")
            {
                ...
                {
                    if (succes_code == 404)
                        response = status_response_html(server, 404, "nofound");
                    else if (succes_code == 500)
                        response = status_response_html(server, 500, "ise");
                    send(client.getClientFd(), response.c_str(), response.length(), 0);
                    close(client.getClientFd());
                    return 1;
                }
            ...
            if(_url == "/delete")
            {
                ...
                if(file.is_open())
                {
                    ...
                    {
                        response = status_response_html(server, 403, "forbidden");
                        send(client.getClientFd(), response.c_str(), response.length(), 0);
                        close(client.getClientFd());
                        return 1;
                    }
        ...
        else if(_content_type == "text/plain\r")// pour requete en brut
        {
        ...
        else if(_content_type.find("multipart/form-data", 0) != std::numeric_limits<size_t>::max())
        {
            ...
            if (begin != line.end() && _url == "/upload")
            {
                ...
                new_file.close();
                response = status_response_html(server, 201, "crok");
            ...
            else
            {
                ...
                //     return 1;
                // }
                response = status_response_html(server, 415, "umt");
                send(client.getClientFd(), response.c_str(), response.length(), 0);
                close(client.getClientFd());
                return 1;
            }
...
std::string Request::status_response_html(const Server & server, int succes_code, std::string mode) 
{
    ...
    else if(succes_code == 200 && mode == "delete")
    {
